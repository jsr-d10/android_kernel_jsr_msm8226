...
/*
*  format: llcon=<mode>,<delay>,<textwrap>,<fb_addr>,<fb_bpp>,<fb_height>,
...
static void llcon_drawglyph8(uint8_t * pixels, uint32_t paint, size_t stride, char c)
{
	size_t x, y;
	uint8_t * glyph;
	uint8_t data;
	...
	stride -= (size_t)llcon.font.width;
	stride *= LLCON_PXL_SIZE;

	glyph = (uint8_t *)llcon.font.data;
	glyph += (size_t)llcon.font.height * (uint8_t)c;

	for (y = 0; y < (size_t)llcon.font.height; y++) {
		data = *glyph++;
		for (x = 0; x < (size_t)llcon.font.width; x++) {
...
static void llcon_drawglyph16(uint8_t * pixels, uint32_t paint, size_t stride, char c)
{
	size_t x, y;
	uint8_t * glyph;
	uint8_t data;
	...
	glyph = (uint8_t *)llcon.font.data;
	glyph += 2 * (size_t)llcon.font.height * (uint8_t)c;

	gw = (size_t)llcon.font.width - 8;
...
inline
uint8_t * get_char_pos_ptr(size_t x, size_t y)
{
	size_t offset;
...
inline
uint8_t * get_cursor_pos_ptr(void)
{
...
void llcon_putc(char c)
{
	...
	llcon.drawglyph(pixels, llcon.fg_color, llcon.display.stride, c);
...
void llcon_putc_to_buf(char c)
{
	...
	if (llcon.log.cur.x >= llcon.res.x) {
		if (c == 0 || c == '\n') {
			atomic_inc(&llcon.log.put_count);
			newline = 1;
			goto exit;
		}
		return;
...
exit:
	...
	atomic_dec(&llcon.log.put_count);
}
...
int llcon_set_font_type(const struct font_desc * font)
{
	...
	if (llcon.font.width <= 8) {
		llcon.drawglyph = llcon_drawglyph8;
	} else
	if (llcon.font.width <= 16) {
		llcon.drawglyph = llcon_drawglyph16;
	} else {
		return -2;
	}
	return 0;
...
int llcon_thread(void * data)
{
	...
	while (llcon_enabled) {
		...
		if (cy == llcon.log.last_line)
		  continue;

		pos = 0;
		if (cy >= llcon.res.y) {
			py = cp / llcon.res.x;
			pos = (py + 1) * llcon.res.x;
			if (pos >= llcon.log.bufsize)
				pos = 0;
		}

		pixels = get_char_pos_ptr(0, 0);
		scanline = llcon.display.stride * LLCON_PXL_SIZE * (size_t)llcon.font.height;
		dx = (size_t)llcon.font.width * LLCON_PXL_SIZE;
		dy = scanline - (dx * llcon.res.x);

		for (y = 0; y < llcon.max.y; y++) {
			for (x = 0; x < llcon.res.x; x++) {
				c = llcon.log.buf[pos++];
				llcon.drawglyph(pixels, llcon.fg_color, llcon.display.stride, c);
				if (pos >= llcon.log.bufsize)
					pos = 0;
...
exit:
	...
	while (atomic_read(&llcon.log.put_count)) {
		mdelay(5);
	}
	mdelay(100);
	llcon_mem_free();
...
int llcon_init(void)
{
	...
	fbsize = llcon.display.height * llcon.display.stride * llcon.display.pixel_size;
